...
statement_json_file format:
   ...
   "question": {
	...
	"passage"? : ""?
	"InputFeatures":{"inputs_id"? : []? ,"input_mask"? : []?, "segment_ids"? : []?}
  }
'''
class data_with_graphs_and_paths(data.Dataset):

	def __init__(self, statement_json_file, graph_ngx_file, pf_json_file, pretrained_sent_vecs, num_choice=4, start=0, end=None, reload=True, cut_off=3):
		...
		for question_id in range(len(statement_json_data)):
			...
			self.passage.append(statement_json_data[question_id]["passage"])
			self.features.append(statement_json_data[question_id]["InputFeatures"])

			for k, s in enumerate(statement_json_data[question_id]["statements"]):
				assert len(statement_json_data[question_id]["statements"]) == num_choice  # 4
				qa_text_cur.append((s["statement"], s['label']))
				if s["label"] is True:  # true of false
					self.correct_labels.append(k)  # the truth id [0,1,2,3]
		...
		self.passage = self.passage[start:end]
		self.features = self.features[start:end]

		assert len(self.statements) == len(self.correct_labels) == len(self.qids) == len(self.passage) == len(self.features)
	...
	def __getitem__(self, index):
		return torch.Tensor([self.statements[index]]), torch.Tensor([self.correct_labels[index]]), self.dgs[index], \
			   ...
			   self.passage[index], torch.Tensor(self.features[index]['input_ids']), torch.Tensor(self.features[index]['input_mask']), torch.Tensor(self.features[index]['segment_ids'])
